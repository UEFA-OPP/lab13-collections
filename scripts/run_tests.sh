...
# ============================================================
# Lab 13 — Inventory & Party (Collections) Test Runner
# ============================================================
...
SRC_DIR="$ROOT_DIR/assignments/collections"
TEST="$ROOT_DIR/tests/CollectionsTest.java"
...
compile_all() {
    mkdir -p "$BUILD_DIR"
    echo -e "${DIM}[1/3] Collections файлуудыг компайл хийж байна...${RESET}"
    javac -d "$BUILD_DIR" "$SRC_DIR"/*.java 2>&1
    if [ $? -ne 0 ]; then
        echo -e "${RED}  ✗ Collections компайл алдаатай!${RESET}"
        exit 1
    fi
    echo -e "${GREEN}  ✓ Collections компайл амжилттай${RESET}"

    echo -e "${DIM}[2/3] CollectionsTest.java компайл хийж байна...${RESET}"
...
main() {
    echo -e "${BOLD}${MAGENTA}"
    echo "  ╔═══════════════════════════════════════════════╗"
    echo "  ║  🐉 Lab 13 — Collections Grader              ║"
    echo "  ╚═══════════════════════════════════════════════╝"
    echo -e "${RESET}"
    ...
    for f in "$SRC_DIR/Inventory.java" "$SRC_DIR/Party.java" "$SRC_DIR/ItemType.java" "$SRC_DIR/Container.java"; do
        if [ -f "$f" ]; then
